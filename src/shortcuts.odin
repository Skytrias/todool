...
task_swap :: proc(manager: ^Undo_Manager, a, b: int) {
	aa := cast(^^Task) &app.mode_panel.children[a]
	bb := cast(^^Task) &app.mode_panel.children[b]
...
task_remove_at_index :: proc(manager: ^Undo_Manager, index: int) {
	item := Undo_Item_Task_Remove_At { index, cast(^Task) app.mode_panel.children[index] }
...
undo_task_insert_at :: proc(manager: ^Undo_Manager, item: rawptr) {
	...
	inject_at(&app.mode_panel.children, data.index, data.task)
...
undo_task_append :: proc(manager: ^Undo_Manager, item: rawptr) {
	...
	append(&app.mode_panel.children, data.task)
...
undo_task_pop :: proc(manager: ^Undo_Manager, item: rawptr) {
	...
	output := Undo_Item_Task_Append { 
		cast(^Task) app.mode_panel.children[len(app.mode_panel.children) - 1],
	}
	pop(&app.mode_panel.children)
...
// 	} else {
// 		data.old_length = len(app.mode_panel.children)
...
issue_copy :: proc() {
	if !app.last_was_task_copy {
		element_repaint(app.mode_panel) // required to make redraw and copy 
	}
...
todool_duplicate_line :: proc(du: u32) {
	...
	copy_state_paste_at(&copy_temp, manager, task.index + 1, lowest_indentation)
...
todool_center :: proc(du: u32) {
	if app.task_head != -1 {
		cam := mode_panel_cam()
		cam.freehand = false
		cam_center_by_height_state(cam, app.mode_panel.bounds, app.caret_rect.t)
	}
}
...
todool_sort_locals :: proc(du: u32) {
	...
	if task_current.visible_parent == nil {
		from = 0
		to = len(app.mode_panel.children) - 1
		// return
	} else {
...
vim_visual_move_left :: proc(du: u32) {
	...
	switch app.mode_panel.mode {
		case .Kanban: {
			...
			if closest_task != nil {
				app.task_head = closest_task.visible_index
				app.task_tail = closest_task.visible_index
				window_repaint(app.window_main)
